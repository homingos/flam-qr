...
export const getContrastLevel = (ratio: number): { level: string; warning: boolean; message: string } => {
  if (ratio >= 7) {
      return { level: "AAA", warning: false, message: "Excellent contrast" };
  } else if (ratio >= 4.5) {
      return { level: "AA", warning: false, message: "Good contrast" };
  } else if (ratio >= 3) {
      return { level: "AA Large", warning: true, message: "Low contrast - may be difficult to scan" };
  } else {
      return { level: "Fail", warning: true, message: "Poor contrast - QR code may not scan properly" };
  }
};